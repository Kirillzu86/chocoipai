  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        try {
            ...
            if (user && user.id) {
                setCurrentUser(user);
                // Загружаем только курсы пользователя
                const myCoursesResponse = await axios.get<Course[]>(`http://localhost:8000/api/v1/users/${user.id}/courses`);
                setMyCourses(myCoursesResponse.data);
            } else {
                setCurrentUser(null);
                localStorage.removeItem("currentUser"); // На всякий случай
                // Для гостя загружаем все курсы (как витрину)
                const allCoursesResponse = await axios.get<Course[]>("http://localhost:8000/api/v1/courses");
